...
//
// Ported from Sub2API: internal/service/account_scheduling_threshold_eval.go
// (EvaluateAccountSchedulingThreshold and supporting functions).
...
//
// This is the core threshold algorithm ported from Sub2API. It:
// 1. Resolves the platform and checks it supports threshold-based pausing
// 2. Determines the effective threshold (account override or global setting)
// 3. Reads platform-specific usage candidates (5h, 7d, quota windows)
// 4. Picks the candidate whose reset is furthest in the future
// 5. Returns a decision with ShouldPause=true if usage >= threshold and window hasn't reset
//
// Pure function — no DB, no side effects. Safe for unit testing.
