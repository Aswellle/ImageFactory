...
func (tq *TagQuery) QueryAssets() *AssetQuery {
...
func (tq *TagQuery) QueryAssetTags() *AssetTagQuery {
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withAssetTags: tq.withAssetTags.Clone(),
...
func (tq *TagQuery) WithAssets(opts ...func(*AssetQuery)) *TagQuery {
	query := (&AssetClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TagQuery) WithAssetTags(opts ...func(*AssetTagQuery)) *TagQuery {
	query := (&AssetTagClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withAssetTags = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withAssetTags; query != nil {
		if err := tq.loadAssetTags(ctx, query, nodes,
			func(n *Tag) { n.Edges.AssetTags = []*AssetTag{} },
			func(n *Tag, e *AssetTag) { n.Edges.AssetTags = append(n.Edges.AssetTags, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadAssets(ctx context.Context, query *AssetQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Asset)) error {
...
func (tq *TagQuery) loadAssetTags(ctx context.Context, query *AssetTagQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *AssetTag)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(assettag.FieldTagID)
	}
	query.Where(predicate.AssetTag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tag.AssetTagsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TagQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withUser != nil {
			_spec.Node.AddColumnOnce(tag.FieldUserID)
		}
	}
