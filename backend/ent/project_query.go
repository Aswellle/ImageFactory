...
func (_q *ProjectQuery) QueryAssets() *AssetQuery {
...
func (_q *ProjectQuery) QueryGenerationJobs() *GenerationJobQuery {
...
func (_q *ProjectQuery) WithAssets(opts ...func(*AssetQuery)) *ProjectQuery {
	query := (&AssetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) WithGenerationJobs(opts ...func(*GenerationJobQuery)) *ProjectQuery {
	query := (&GenerationJobClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) loadAssets(ctx context.Context, query *AssetQuery, nodes []*Project, init func(*Project), assign func(*Project, *Asset)) error {
...
func (_q *ProjectQuery) loadGenerationJobs(ctx context.Context, query *GenerationJobQuery, nodes []*Project, init func(*Project), assign func(*Project, *GenerationJob)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
