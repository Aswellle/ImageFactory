...
func (pq *ProjectQuery) QueryAssets() *AssetQuery {
...
func (pq *ProjectQuery) QueryGenerationJobs() *GenerationJobQuery {
	query := (&GenerationJobClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withGenerationJobs: pq.withGenerationJobs.Clone(),
...
func (pq *ProjectQuery) WithAssets(opts ...func(*AssetQuery)) *ProjectQuery {
	query := (&AssetClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *ProjectQuery) WithGenerationJobs(opts ...func(*GenerationJobQuery)) *ProjectQuery {
	query := (&GenerationJobClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withGenerationJobs = query
	return pq
}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withAssets; query != nil {
		if err := pq.loadAssets(ctx, query, nodes,
			func(n *Project) { n.Edges.Assets = []*Asset{} },
			func(n *Project, e *Asset) { n.Edges.Assets = append(n.Edges.Assets, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withGenerationJobs; query != nil {
		if err := pq.loadGenerationJobs(ctx, query, nodes,
			func(n *Project) { n.Edges.GenerationJobs = []*GenerationJob{} },
			func(n *Project, e *GenerationJob) { n.Edges.GenerationJobs = append(n.Edges.GenerationJobs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadAssets(ctx context.Context, query *AssetQuery, nodes []*Project, init func(*Project), assign func(*Project, *Asset)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(asset.FieldProjectID)
	}
	query.Where(predicate.Asset(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.AssetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (pq *ProjectQuery) loadGenerationJobs(ctx context.Context, query *GenerationJobQuery, nodes []*Project, init func(*Project), assign func(*Project, *GenerationJob)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(generationjob.FieldProjectID)
	}
	query.Where(predicate.GenerationJob(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.GenerationJobsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
