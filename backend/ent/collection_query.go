...
func (cq *CollectionQuery) QueryAssets() *AssetQuery {
...
func (cq *CollectionQuery) FirstID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (cq *CollectionQuery) FirstIDX(ctx context.Context) int64 {
...
func (cq *CollectionQuery) OnlyID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (cq *CollectionQuery) OnlyIDX(ctx context.Context) int64 {
...
func (cq *CollectionQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (cq *CollectionQuery) IDsX(ctx context.Context) []int64 {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CollectionQuery) WithAssets(opts ...func(*AssetQuery)) *CollectionQuery {
	query := (&AssetClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(collection.FieldUserID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (cq *CollectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Collection, error) {
	...
	if query := cq.withUser; query != nil {
		if err := cq.loadUser(ctx, query, nodes, nil,
			func(n *Collection, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CollectionQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Collection)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (cq *CollectionQuery) loadAssets(ctx context.Context, query *AssetQuery, nodes []*Collection, init func(*Collection), assign func(*Collection, *Asset)) error {
	...
	query.Where(predicate.Asset(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(collection.AssetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.collection_assets
		if fk == nil {
			return fmt.Errorf(`foreign-key "collection_assets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "collection_assets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CollectionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(collection.Table, collection.Columns, sqlgraph.NewFieldSpec(collection.FieldID, field.TypeInt64))
	_spec.From = cq.sql
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withUser != nil {
			_spec.Node.AddColumnOnce(collection.FieldUserID)
		}
	}
...
func (cq *CollectionQuery) ForUpdate(opts ...sql.LockOption) *CollectionQuery {
...
func (cq *CollectionQuery) ForShare(opts ...sql.LockOption) *CollectionQuery {
	if cq.driver.Dialect() == dialect.Postgres {
		cq.Unique(false)
	}
	cq.modifiers = append(cq.modifiers, func(s *sql.Selector) {
